...
export default function TravelMatch() {
      ...
      <div className="p-6 max-w-md mx-auto space-y-4">
        ...
        {res.map((r, i) => (
          <div key={i} className="border p-4 rounded">
            ...
            <p>{r.description}</p>
            <p className="text-sm italic text-gray-500">{r.country}</p>
            <img
              src={r.image}
              alt={r.name}
            ...
            />
            <a
              href={`https://www.google.com/search?q=travel+to+${r.name}`}
        ...
        <button
          onClick={() => {
            setR(null)
            setP({ beaches: false, history: false, mountains: false, city: false })
          }}
          className="btn-simple"
        >
          Reset
        </button>
        <button onClick={() => nav('/')} className="btn-simple">
          Back
        </button>
      </div>
    )
  ...
  return (
    <div className="p-6 max-w-md mx-auto space-y-4">
      ...
      <button onClick={find} className="btn-simple">
        Find Matches
      </button>
      <button onClick={() => nav('/')} className="btn-simple">
        Back
